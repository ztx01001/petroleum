...
public class CRCUtil {
	...
	/**
	 * ModBus 通信协议的CRC（冗余循环校验码含2个字节，即16位二进制数）
	 * 计算CRC16校验码
	 * @param byte[] 字节数组
	 * @return byte[] 字节数组
	 * 
	 **/
	public static byte[] getCRCModbus(byte[] bytes) {
		int CRC = 0x0000ffff;
        ...
        //大端序存储04 00 01验证，计算出的CRC16低字节位DB，高字节位4B
        byte[] crcByte = ByteUtil.int2Bytes(CRC);
        //把大端序的字节数组转换成小端序，并只要后两位
        byte[] crcResult = new byte[2];
        System.arraycopy(crcByte, 2, crcResult, 1, 1);
        System.arraycopy(crcByte, 3, crcResult, 0, 1);
